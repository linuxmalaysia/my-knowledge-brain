---
title: "Tutorials Index"
sidebarTitle: "Tutorials Index"
description: "Welcome to the Deep State of Mind (DSOM) Tutorials quadrant, structured according to the Diátaxis Framework. Deep State of Mind framework."
---

Welcome to the **Deep State of Mind (DSOM) Tutorials** quadrant, structured according to the **Diátaxis Framework**.

## Learning-oriented tutorials

Tutorials are **guided, step-by-step learning lessons** designed to help beginners explore the DSOM toolchain through hands-on execution.

- **[Getting Started with DSOM Tools](getting-started):** A beginner-friendly, end-to-end tutorial on initializing the OpenWiki, validating frontmatter compliance, and starting the local FastMCP server.
- **[Team Masterclass: From Zero to Sovereign AI Pair-Programming](TEAM-DSOM-MASTERCLASS.md):** A comprehensive operational curriculum teaching teams how to bootstrap projects, configure dual GitOps (GitHub + GitLab), pair-program with Antigravity, and collaborate with Google Jules.
- **[Nota Lapangan: Panduan Praktikal Pasukan untuk Antigravity & DSOM](NOTA-LAPANGAN-ANTIGRAVITY-DSOM):** Nota ringkas dan panduan praktikal perkongsian Cikgu Haris mengenai strategi model, automasi kemahiran (skills), dan disiplin Ansible/GitOps.

---
*Deep State of Mind (DSOM) For My AI Protocol | Harisfazillah Jamel (LinuxMalaysia) | 2026-08-22*
*Standard: UK English | DBP-standard Bahasa Melayu Malaysia (Piawai) | GNU General Public License v3.0*
